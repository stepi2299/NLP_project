...
from src.classifiers.transformer_based.bert_factory import BertFactory
...
class BertClassifierInterface:
    def __init__(self, model_name, dropout_prob, n_classes, lr=1e-5, wg=None):
        ...
        self.tokenizer, self.model = BertFactory.create(
            model_name, dropout_prob, n_classes
        )
    ...
    def data_preprocess(
        ...
        train_dataset = MeldDataset(
            df_train, tokenizer, x_label, y_label, max_length, transform
        )
        val_dataset = MeldDataset(
            df_val, tokenizer, x_label, y_label, max_length, transform
        )
    ...
    def train(
        ...
        train_data_loader, val_data_loader, self.mapper = self.data_preprocess(
            df, self.tokenizer, x_label, y_label, max_length, batch_size, transform
        )
        total_steps: int = len(train_data_loader) * epochs
        scheduler = get_linear_schedule_with_warmup(
            optimizer=self.optimizer, num_warmup_steps=0, num_training_steps=total_steps
        )
        best_acc: float = 0
        for epoch_i in range(epochs):
            ...
            train_acc, train_loss = self._train(
                data_loader=train_data_loader, scheduler=scheduler
            )
